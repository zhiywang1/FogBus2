...
if __name__ == "__main__":
    ...
    # exit()
    # start mariadb
    db = Base('aws', 'docker start fogbus2-mariadb-new', 'docker stop fogbus2-mariadb-new', 'fogbus2-mariadb')
    db.stop_all()
    db.start()
    sleep(3)
    ...
    remote_logger = Base('aws',
                         'cd ~/FogBus2New/containers/remoteLogger && '
    ...
    # start master
    master = Base('rpi-a',
                  'cd ~/FogBus2New/containers/master && '
                  'docker compose run --rm -d '
    ...
    actor = Base('nectar-k8s32vcpu30gb',
                 'cd ~/FogBus2New/containers/actor && '
    ...
    for i in tqdm(range(200)):
        user = Base('rpi-d',
                    ...
                    '--keyFile server.key '
                    '--domainName fogbus2',
                    'docker rm -f $(docker ps | grep User | cut -c 1-10)', 'User')
        user.stop_all()
        user.start()
        sleep(20)
        user.stop()
        ...
        parse_delays(rows, delays)
        save_delays(delays, f'delays-tls-overlay-{time()}.csv')
    actor.stop()
    master.stop()
    remote_logger.stop()
    db.stop()
